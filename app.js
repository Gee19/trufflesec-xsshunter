...
async function get_app_server() {
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      if (process.env.USE_CLOUD_STORAGE == "true") {
        ...
        console.debug(
          `${payload_fire_image_id}.png.gz has been uploaded to GCS.`
        );
      ...
      if (encrypted) {
        if (req.body.encrypted_data.length > 100000) {
          Sentry.captureMessage(
            `encrypted data length too long: ${req.body.encrypted_data.length}`
          );
          return res
            .status(400)
            .json({
          ...
          Sentry.captureMessage(
            `public key  length too long: ${req.body.pgp_key.length}`
          );
          return res
            .status(400)
            .json({
      ...
      const new_payload_fire_result = await database.savePayload(
        payload_fire_data
      );
      ...
      if (
        ...
        await notification.send_email_notification(
          payload_fire_data,
          user.email
        );
      }
  ...
  const payload_handler = async (req, res) => {
    ...
    res.send(
      XSS_PAYLOAD.replace(/\[HOST_URL\]/g, xssURI)
        .replace("[COLLECT_PAGE_LIST_REPLACE_ME]", JSON.stringify([]))
        .replace(/\[USER_PATH\]/g, userPath)
        .replace("[pgp_key]", pgp_key)
